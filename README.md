...
By default everything runs on the user side: you can open the app directly in the browser and it stores data locally using `localStorage`. **Optional cloud sync:** this deployment ships `supabase-config.js` so [GitHub Pages](https://pages.github.com/) visitors can sign in and sync across devices (the anon key is public-by-design; access is enforced by Row Level Security in `supabase/schema.sql`). To use your own Supabase project, replace the values in `supabase-config.js` or start from `supabase-config.example.js`.
